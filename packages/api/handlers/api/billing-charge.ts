...
export const handler: APIGatewayProxyHandler = async (event) => {
  try {
    ...
    const provider_type = body.provider_type as string | undefined;
    const amount_cents = body.amount_cents as number | undefined;
    const currency = (body.currency as string) || 'usd';
    const description = body.description as string | undefined;
    const billing_reference = body.billing_reference as string | undefined;
    ...
    await sendSlackAlert('Billing Event Failed', 'critical', [
      { label: 'Error', value: 'Billing event processing failed — see CloudWatch for details' },
    ]);
