...
export const handler: APIGatewayProxyHandler = async (event) => {
  try {
    ...
    // 3. Query locally-stored fax records from DynamoDB (paginated)
    const localResult = await queryItemsPaginated({
      IndexName: 'GSI1',
    ...
    const localFaxes = localResult.items;
    ...
    const nextToken = localResult.lastEvaluatedKey
      ? Buffer.from(JSON.stringify(localResult.lastEvaluatedKey)).toString('base64')
      : null;

    return success({ faxes: allFaxes, nextToken });
  } catch (err) {
