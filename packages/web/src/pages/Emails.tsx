...
import { Mail, Inbox, CheckCircle2 } from 'lucide-react'
import { listEmails } from '../api/endpoints'
        ...
        <Card className="p-0">
          <ul className="divide-y divide-light-gray dark:divide-gray-700">
            {list.map((e) => (
              <li key={e.id}>
                <button
                  onClick={() => tab !== 'Attached' && setAttachEmail(e)}
                  disabled={tab === 'Attached'}
                  className="w-full text-left px-4 py-3 flex items-start gap-3 hover:bg-light-gray/50 dark:hover:bg-gray-700/40 transition-colors disabled:hover:bg-transparent disabled:cursor-default"
                >
